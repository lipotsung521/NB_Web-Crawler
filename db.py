...
client.admin.authenticate("lipotsung", "890521")
'''

#TODO:DB不用分兩個，拆成不同collection就好
...
#連線淨值DB
def constructor_networth():
    client = MongoClient('localhost', 27017)
    client.admin.authenticate("lipotsung", "890521")
    db = client[networthDB]
    return db
...
def insert_article(df):
    ...
    records = df.to_dict('records') #dataframe轉dict
...
def delete_article():
    db = constructor_article()
    collection = db['article']
    collection.delete_many({'category': 'CIO 每週觀點'})
...
#更改文章category原本為影片變成12345
def update_article():
    db = constructor_article()
    collection = db['article']
    myquery = { "category": "影片" }
    newvalues = { "$set": { "category": "12345" }}
    collection.update_one(myquery, newvalues)
#--------------------------------------------------------------------------------  
#新增全部國內淨值到資料庫 
def insert_networthin(df):
    db = constructor_article()
    collection = db['networth']
    records = df.to_dict('records') #dataframe轉dict
    collection.insert_many(records)
#--------------------------------------------------------------------------------  
#新增全部國外淨值到資料庫 
def insert_networthoff(df):
    db = constructor_article()
    collection = db['networth']
    records = df.to_dict('records') #dataframe轉dict
    collection.insert_many(records)
